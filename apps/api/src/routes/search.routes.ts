...
searchRoutes.get('/popular', async (req, res) => {
  ...
  res.json(
    await cached('popular-searches', pincode, {}, 60, () =>
      prisma.searchTrend.findMany({
        where: {
          pincode,
          updatedAt: { gt: new Date(Date.now() - 30 * 86400000) },
        },
        select: { term: true },
...
searchRoutes.post('/event', async (req, res) => {
  ...
  if (redis) {
    const key = createHash('sha256')
      .update(
        [req.ip, b.pincode, term, new Date().toISOString().slice(0, 10)].join(
          '|',
        ),
      )
      .digest('hex');
    try {
      ...
      if (
        ...
        (await prisma.product.findFirst({
          where: { id: b.productId, deletedAt: null },
        }))
      ) {
        await redis.zincrby(`gc:popular-products:${b.pincode}`, 1, b.productId);
        await redis.expire(`gc:popular-products:${b.pincode}`, 30 * 86400);
      }
    } catch {
      res.status(204).end();
      return;
  ...
  await prisma.searchTrend.upsert({
    where: { pincode_term: { pincode: b.pincode, term } },
    create: { pincode: b.pincode, term },
...
searchRoutes.get('/popular-products', async (req, res) => {
  ...
  try {
    ids =
      (await redis?.zrevrange(`gc:popular-products:${pincode}`, 0, 9)) || [];
  } catch {
    /* Optional suggestions. */
  }
  res.json(
    await cached('popular-products', pincode, ids, 30, () =>
