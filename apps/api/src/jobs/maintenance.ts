...
export async function cleanup() {
  const now = new Date(),
    days = (n: number) => new Date(Date.now() - n * 86400000);
  ...
  const results = await prisma.$transaction([
    ...
    prisma.retailerOffer.deleteMany({
      where: { observedAt: { lt: days(30) } },
    }),
  ]);
  return results.map(r => r.count);
}
export async function snapshotPage(cursor?: string) {
  const rows = await prisma.retailerOffer.findMany({
...
export async function reconcilePage(cursor?: string) {
  ...
  for (const row of rows) {
    const areas = await prisma.$transaction(async tx => {
      ...
      const mismatch =
        !listing.canonicalId ||
        (await tx.retailerOffer.findFirst({
          where: {
            retailer: listing.retailer,
            sku: listing.sku,
            inStock: true,
            productId: { not: listing.canonicalId },
          },
          select: { id: true },
        }));
      if (!mismatch) return [];
      ...
      await tx.auditLog.create({
        data: {
          action: 'MAPPING_QUARANTINED',
          objectType: 'RetailerProduct',
          objectId: row.id,
          reason: 'Canonical mismatch detected during reconciliation',
        },
      });
      return tx.retailerOffer.findMany({
        where: { retailer: listing.retailer, sku: listing.sku },
