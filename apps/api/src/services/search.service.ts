...
export async function searchCatalog(q: {
  ...
  const fingerprint = createHash('sha256')
    .update(JSON.stringify([q.location, search, q.category || '', q.limit]))
    .digest('hex')
    .slice(0, 16);
  let offset = 0;
  if (q.cursor) {
    try {
      ...
      throw new ApiError(
        400,
        'INVALID_CURSOR',
        'Search cursor is invalid for this query',
      );
    }
  }
  ...
  return cached(
    ...
    async () => {
      const aliases = await prisma.searchAlias.findMany({
        where: { term: { in: [search, ...search.split(' ')] } },
        take: 30,
      });
      const expansions = new Map(aliases.map(a => [a.term, a.replacement]));
      const query =
        expansions.get(search) ||
        search
          .split(' ')
          .map(t => expansions.get(t) || t)
          .join(' ');
      const doc = Prisma.sql`(p.name || ' ' || p.brand || ' ' || p.category)`;
      const serving = Prisma.join(
        stores.map(
          s =>
            Prisma.sql`(o.retailer=${s.retailer} AND o."storeId"=${s.storeId} AND o."sellerId"=${s.sellerId})`,
        ),
        ' OR ',
      );
      ...
      WHERE p."deletedAt" IS NULL ${
        q.category ? Prisma.sql`AND p.category=${q.category}` : Prisma.empty
      }
      ...
      AND EXISTS (SELECT 1 FROM "RetailerOffer" o JOIN "RetailerProduct" rp ON rp.retailer=o.retailer AND rp.sku=o.sku
        WHERE o."productId"=p.id AND o.location=${
          q.location
        } AND o."inStock" AND o."stockQuantity">0
        AND rp."deletedAt" IS NULL AND rp.status='MATCHED' AND o."observedAt">${new Date(
          Date.now() - env.OFFER_MAX_AGE_SECONDS * 1000,
        )} AND o."observedAt"<=${new Date()} AND (${serving}))
      ...
      return {
        products: rows.slice(0, q.limit).map(p => ({ ...p, variants: [] })),
        nextCursor:
          rows.length > q.limit && offset + q.limit <= 10000
            ? Buffer.from(
                JSON.stringify({ f: fingerprint, o: offset + q.limit }),
              ).toString('base64url')
            : null,
      };
    },
