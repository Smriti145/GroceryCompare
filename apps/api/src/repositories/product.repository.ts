...
import { Product, PLATFORMS, Platform } from '../../../../packages/contracts';
...
export function toProduct(row: ProductRecord): Product {
  return {
    ...
    variants: row.variants
      .filter(v => PLATFORMS.includes(v.platform as Platform))
      .map(v => ({
        id: v.id,
        platform: v.platform as Platform,
        platformName: v.platformName,
        quantity: v.quantity,
        pricePaise: v.price.mul(100).toNumber(),
...
function offers(location: string) {
  return { location, ...(env.ALLOW_DEMO_DATA ? {} : { isDemo: false }) };
}
const offerOrder: Prisma.VariantOrderByWithRelationInput[] = [
  { platform: 'asc' },
  { price: 'asc' },
  { id: 'asc' },
];
export async function getAllProducts(
  ...
  const rows = await prisma.product.findMany({
    ...
    take: query.limit + 1,
    include: {
      variants: { where: offers(query.location), orderBy: offerOrder },
    },
  });
...
export async function getProductById(id: string, location: string) {
  const row = await prisma.product.findFirst({
    where: { id, variants: { some: offers(location) } },
    include: { variants: { where: offers(location), orderBy: offerOrder } },
  });
