...
import { productQuerySchema, productIdSchema, locationSchema } from '../domain/validation';
import { ApiError } from '../middleware/errors';
export async function getProducts(req: Request, res: Response) {
  const query = productQuerySchema.parse(req.query);
  res.json({ success: true, data: await repository.getAllProducts(query), requestId: res.locals.requestId });
}
